...
//! and is invoked by the dispatch logic in `main::run`. Helpers used only by
//! handlers (DNS-01 hook orchestration, EAB parsing, wildcard checks,
//! challenge-status terminality, the hook env contract) live here too.
//!
//! Cross-module helpers that are also used by the dispatch / config code in
//! main.rs (cert reading, account-key handling, CSR generation, HTTP client
//! construction) remain in main.rs and are imported here via the
//! `crate::` paths below.
...
use crate::{
    ...
    load_account_key_with_password, normalize_identifier, out, outln, pem_to_der,
