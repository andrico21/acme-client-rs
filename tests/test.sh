...
SILENT_JSON_CERT="${WORK_DIR}/silent-json-cert.pem"
SILENT_JSON_KEY="${WORK_DIR}/silent-json-key.pem"
SILENT_JSON_DOMAIN="silent-json.${TEST_DOMAIN}"
OUTPUT=$(acme --silent --output-format json --account-key "${JSON_E2E_KEY}" run \
  --contact e2e@example.com \
  --challenge-type http-01 \
  --http-port 5002 \
  --cert-output "${SILENT_JSON_CERT}" \
  --key-output "${SILENT_JSON_KEY}" \
  "${SILENT_JSON_DOMAIN}" 2>/dev/null)
RC=$?
if [[ ${RC} -eq 0 ]]; then
  if [[ -z "${OUTPUT}" ]]; then
    pass "Silent JSON run produced no stdout"
  else
    fail "79b" "Silent JSON run produced stdout: ${OUTPUT}"
  fi
else
  fail "79b" "Silent JSON run failed (exit ${RC})"
fi
rm -f "${SILENT_JSON_CERT}" "${SILENT_JSON_KEY}" 2>/dev/null

# 79c: silent generate-key
...
if [[ ${RC} -eq 0 ]]; then
  if [[ -z "${OUTPUT}" ]]; then
    ...
    fail "79c" "Silent generate-key produced stdout: ${OUTPUT}"
  fi
  if [[ -f "${SILENT_GEN_KEY}" ]]; then
    pass "Key file created despite --silent"
  else
    fail "79c" "Key file not created"
  fi
else
  fail "79c" "Silent generate-key failed (exit ${RC})"
fi
rm -f "${SILENT_GEN_KEY}" 2>/dev/null

# 79d: silent show-config
OUTPUT=$(acme --silent show-config 2>/dev/null)
RC=$?
if [[ ${RC} -eq 0 ]] && [[ -z "${OUTPUT}" ]]; then
  pass "Silent show-config produced no stdout"
else
  fail "79d" "Silent show-config produced stdout or failed (exit ${RC})"
fi
