...
graph TD
    ...
    %% Print cert to stdout (opt-in)
    SAVE_CERT --> PRINT_CERT{"--print-cert?"}
    PRINT_CERT -- Yes --> PRINT_PEM["Print PEM to stdout"]
    PRINT_PEM --> CERT_HOOK
    PRINT_CERT -- No --> CERT_HOOK

    %% Post-issue hook
    CERT_HOOK{"--on-cert-issued<br/>hook?"}
