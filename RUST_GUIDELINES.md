...
For library crates, public API surface must be future-proof and
documented (`atlassian-mcp` is a binary crate, so these lints apply
only to upstream framework code in `rmcp-server-kit`).
